...
def sep_layer(inputs, filters, strides, activation=True, name=None,dilation_rate=(1,1)):
    sep_conv = keras.layers.SeparableConv2D(
        filters, 3, strides=strides, padding='same', name=name, dilation_rate=dilation_rate)(inputs)
    if name is not None:
        batch_name = name+'_batch_norm'
        relu_name = name+'_relu'
    else:
...
def exit_block(inputs):
    ...
    sep_conv3 = sep_layer(sep_conv2, 1024, 1,
                          name='exit_sep_conv3', activation=False)
    # Skip Convolution
    conv1 = conv_layer(inputs, 1024, 1, 1, 'same',
    ...
    sep_conv6 = sep_layer(sep_conv5, 2048, 1, name='exit_sep_conv6', dilation_rate=(2,2))
